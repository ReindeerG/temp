...
public class Mainwindow extends JFrame {
	private Client_Ex client=null;
	private ArrayList<Player> players;
	public void setClient(Client_Ex client) {
		this.client=client;
	}
...
//			img_bt_invite_on = new ImageIcon(ImageIO.read(new File("Images\\invite_on.png")));
	...
	public void Refresh() {
		...
		for(Player p : players) {
			...
			if(p.getUser().getId().equals(client.getUser().getId())) {
				client.setMe(p);
			}
		}
		if(client.isInggame()==false) {
		...
		for(Player p : players) {
			if((p.getOrder()%4)==((temporder+1)%4)) {
				...
				if(p.getOrder()==0) { Lbl_2p_nick.setText("<"+p.getUser().getNickname()+"> (방장)"); }
				else { name_2p=p.getUser().getNickname(); userid_2p=p.getUser().getId(); Lbl_2p_nick.setText("<"+name_2p+">"); }
				Lbl_2p_money.setText("가진 돈: "+p.getUser().getMoney()+"전");
				if(p.getReady()==0) {
					Lbl_2p_ready.setText("");
				} else {
			...
			if((p.getOrder()%4)==((temporder+2)%4)) {
				...
				if(p.getOrder()==0) { Lbl_3p_nick.setText("<"+p.getUser().getNickname()+"> (방장)"); }
				else { name_3p=p.getUser().getNickname(); userid_3p=p.getUser().getId(); Lbl_3p_nick.setText("<"+name_3p+">"); }
				Lbl_3p_money.setText("가진 돈: "+p.getUser().getMoney()+"전");
				if(p.getReady()==0) {
					Lbl_3p_ready.setText("");
				} else {
			...
			if((p.getOrder()%4)==((temporder+3)%4)) {
				...
				if(p.getOrder()==0) { Lbl_4p_nick.setText("<"+p.getUser().getNickname()+"> (방장)"); }
				else { name_4p=p.getUser().getNickname(); userid_4p=p.getUser().getId(); Lbl_4p_nick.setText("<"+name_4p+">"); }
				Lbl_4p_money.setText("가진 돈: "+p.getUser().getMoney()+"전");
				if(p.getReady()==0) {
					Lbl_4p_ready.setText("");
				} else {
...
//			else {
	...
	//			} else
				...
				} else if(client.getMe().getUser().getMoney()<client.getMinforbet()) {
					bt_bet_call.setIcon(img_bt_call_no);
					bt_bet_call.setRolloverIcon(img_bt_call_no);
				...
				else if(client.getMe().getUser().getMoney()<client.getMoneythisgame()/2) {
					bt_bet_half.setIcon(img_bt_half_no);
					bt_bet_half.setRolloverIcon(img_bt_half_no);
...
//	}
	...
	public Mainwindow(Client_Ex client, ObjectOutputStream out, MainWindow mw) {
		this.client=client;
		this.out = out;
		this.mw = mw;
